...
searchBar.addEventListener('keyup', (e) => {
    ...
    const filteredCharacters = hpCharacters.filter((character) => {
        return (
            ...
            character.brand.toLowerCase().includes(searchString)
        );
    });
...
const displayCharacters = (characters) => {
    ...
    const htmlString = characters
        .map((character) => {
            if(character.rating>4.3){
            return `
            <li class="character">
            
            <div class="stars inline"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-fill" viewBox="0 0 16 16">
          ...
          <p>Rating:${character.rating}
          </div>
          
                <h2>${character.title}</h2>
                <h3>Price:$ ${character.price}-${character.discountPercentage}%</h3>
                <p>${character.description}</p>
                <img src="${character.thumbnail}" class="imge"></img>
